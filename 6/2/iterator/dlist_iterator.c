 ...
 * =====================================================================================
 *
 *       Filename:  dlist_iterator.c
 *
 *    Description:  dlist_iterator c source file 
 *
 *        Version:  1.0
 *        Created:  03/06/2019 10:18:16 AM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
static Ret dlist_iteator_set(Iterator* thiz,void* data)
{
...
static Ret dlist_iteator_get(Iterator* thiz,void** data)
{
    PrivInfo* priv = (PrivInfo*)thiz->priv;
...
static Ret dlist_iteator_advance(Iterator* thiz,int offset)
{
    int n = offset;
    Ret ret = RET_FAIL;
    DListNode* iter = NULL;
...
static Ret dlist_iteator_clone(Iterator* thiz,Iterator** cloned)
{
    PrivInfo* priv = (PrivInfo*)thiz->priv;
    return_val_if_fail(priv->cursor != NULL && priv->dlist != NULL,RET_INVALID_PARAMS);
...
Iterator* dlist_iterator_create(DList* dlist)
{
    ...
    if((thiz = malloc(sizeof(Iterator) + sizeof(PrivInfo))) != NULL)
    {
        ...
        thiz->set = dlist_iteator_set;
        thiz->get = dlist_iteator_get;
        thiz->next = dlist_iterator_next;
        thiz->prev = dlist_iterator_prev;
        thiz->advance = dlist_iteator_advance;
        thiz->clone = dlist_iteator_clone;
