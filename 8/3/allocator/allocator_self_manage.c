 ...
 * =====================================================================================
 *
 *       Filename:  allocator_self_manage.c
 *
 *    Description:  allocator_self_manage c source file 
 *
 *        Version:  1.0
 *        Created:  04/22/2019 04:23:02 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
static void allocator_self_manage_merge(Allocator* thiz,FreeNode* iter)
{
    ...
    if(next != NULL && ((sizeof)iter + iter->length) == (size_t)next)
    {
...
static void allocator_self_manage_free(Allocator* thiz,void* ptr)
{
    ...
    allocator_self_manage(thiz,free_iter);
...
static void allocator_self_manage_dump(Allocator* thiz)
{
    int i = 0;
    FreeNode* iter = NULL;
    PrivIfo* priv = (PrivIfo*)thiz->priv;
...
int main(int argc,char*argv[])
{
    ...
    for(i = 0; i < n/3; i++)
    {
        ...
        ptr = allocator_calloc(allocator,ptr);
    }

    for(i = 0; i < sizeof(ptrs)/sizeof(ptrs[0]); i++)
    {
        ptrs[i] = allcator_calloc(allocator,1,i);
        //ptrs[i] = allcator_realloc(allocator,ptrs[i],2*i);
    }
