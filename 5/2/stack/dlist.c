 ...
 *
 *        Version:  1.0
 *        Created:  02/28/2019 10:20:17 AM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
static void test_int_dlist(void)
{
    ...
    for(i = 0; i < n; i++)
    {
        ...
        assert(dlist_get_by_index(dlist,index,(void**)&data) == RET_OK);
        assert(data == 2*i);
        assert(dlist_set_by_index(dlist,i,(void*)i) == RET_OK);
        assert(dlist_find(dlist,cmp_int,(void*)i) == RET_OK);
    }
...
static void multi_thread_test(void)
{
    ...
    printf("length = %d",dlist_length(dlist));
