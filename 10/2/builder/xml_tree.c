 ...
 *       Filename:  xml_tree.c
 *
 *    Description:  xml_tree c source file 
 *
 *        Version:  1.0
 *        Created:  04/30/2019 09:31:57 AM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
XmlNode* xml_node_create_normal(const char* name,const char** attrs)
{
    ...
    if((node = calloc(1,sizeof(XmlNode))) != NULL)
    {
        ...
        if(attrs != NULL)
        {
            for(i = 0;attrs[i] != NULL && attrs[i+1] != NUL; i += 2)
            {
                xml_node_append_attr(node,attrs[i],attrs[i+1]);
            }
        }
...
{
    const char* attrs[] = {"name","xxx","age","100",NULL};
    ...
    assert(strcpm(pi.u.normal.name,"xml") == 0);
