 ...
 *
 *        Version:  1.0
 *        Created:  04/25/2019 02:12:36 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
static void xml_parser_parse_satrt_tag(XmlParser* thiz);
...
void xml_parser_parse(XmlParser* thiz,const char* xml)
{
    ...
    for(; *thiz->read_ptr != '\0'; thiz->read_ptr++)
    {
        ...
        switch(state)
        {
            ...
            case STAT_START_TAG:
                {
                    xml_parser_parse_satrt_tag(thiz);
                    state = STAT_NONE;
                    break;
                }
...
#define XML "<?xml version=\"1.0\" encoding=\"utf-8\"?> \
    ...
    <programmer name=\"tentstan\" blog=\"http://www.baidu.com/blog\">text</programmer>"
